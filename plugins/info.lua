...
local function setrank(msg, name, value) -- setrank function
  ...
  if hash then
	...
	return send_msg('chat#id'..msg.to.id, '„ﬁ«„ ò«—»— ('..name..') »Â '..value..'  €ÌÌ— œ«œÂ ‘œ ', ok_cb,  true)
  end
end
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = '‰œ«—œ'
    ...
    local text = '‰«„ ò«„· : '..(result.first_name or '')..' '..(result.last_name or '')..'\n'
               ..'ÌÊ“—: '..Username..'\n'
               ..'«ÌœÌ ò«—»—Ì : '..result.id..'\n\n'
    ...
    if not value then
	 if result.id == tonumber(SUDO) then
	   text = text..'„ﬁ«„ : „œÌ— ò· —»«  (Executive Admin) \n\n'
	  elseif is_admin2(result.id) then
	   text = text..'„ﬁ«„ : «œ„Ì‰ —»«  (Admin) \n\n'
	  elseif is_owner2(result.id, extra.chat2) then
	   text = text..'„ﬁ«„ : „œÌ— ò· ê—ÊÂ (Owner) \n\n'
	  elseif is_momod2(result.id, extra.chat2) then
	    text = text..'„ﬁ«„ : „œÌ— ê—ÊÂ (Moderator) \n\n'
      else
	    text = text..'„ﬁ«„ : ò«—»— (Member) \n\n'
	 end
   else
   text = text..'„ﬁ«„ : '..value..'\n\n'
  end
  ...
  text = text..' ⁄œ«œ ÅÌ«„ Â«Ì ›—” «œÂ ‘œÂ : '..user_info_msgs..'\n\n'
  text = text..'?Alominateam?'
  send_msg(extra.receiver, text, ok_cb,  true)
  else
	send_msg(extra.receiver, extra.user..' ‰«„ ò«—»—Ì „Ê—œ ‰Ÿ— Ì«›  ‰‘œ.', ok_cb, false)
  end
end
...
local function action_by_id(extra, success, result)  -- /info <ID> function
 ...
 if result.username then
  ...
  send_msg(extra.receiver, '«ÌœÌ ‘Œ’ „Ê—œ ‰Ÿ— œ— ”Ì” „ À»  ‰‘œÂ «” .\n«“ œ” Ê— “Ì— «” ›«œÂ ò‰Ìœ\n/info @username', ok_cb, false)
  end
end
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
    ...
    local text = '‰«„ ò«„· : '..(result.from.first_name or '')..' '..(result.from.last_name or '')..'\n'
               ..'ÌÊ“—: '..Username..'\n'
               ..'«ÌœÌ ò«—»—Ì : '..result.from.id..'\n\n'
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = '‰«„ : '..(msg.from.first_name or '‰œ«—œ')..'\n'
   local text = text..'›«„Ì· : '..(msg.from.last_name or '‰œ«—œ')..'\n'	
   local text = text..'ÌÊ“— : '..Username..'\n'
   local text = text..'«ÌœÌ ò«—»—Ì : '..msg.from.id..'\n\n'
